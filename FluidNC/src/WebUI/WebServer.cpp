...
//embedded response file if no files on LittleFS
#    include "NoFile.h"
...
#    endif
        ...
        //LittleFS
        _webserver->on("/files", HTTP_ANY, handleFileList, LittleFSFileupload);
...
#    endif
    ...
    void Web_Server::handle_root() {
        ...
        if ((LittleFS.exists(pathWithGz) || LittleFS.exists(path)) && !_webserver->hasArg("forcefallback") &&
    ...
    //Handle not registred path on LittleFS neither SD ///////////////////////
...
#    endif
    ...
    //LittleFS
    //LittleFS files list and file commands
    void Web_Server::handleFileList() {
            ...
            if (_webserver->arg("action") == "delete" && _webserver->hasArg("filename")) {
                ...
                if (!LittleFS.exists(filename)) {
                    ...
                    if (LittleFS.remove(filename)) {
                        ...
                        File dir        = LittleFS.open(ptmp);
                        File dircontent = dir.openNextFile();
                        if (!dircontent) {
                            //keep directory alive even empty
                            File r = LittleFS.open(path + "/.", FILE_WRITE);
                            if (r) {
                                r.close();
                            }
                        }
            ...
            if (_webserver->arg("action") == "deletedir" && _webserver->hasArg("filename")) {
                ...
                if (filename != "/") {
                    if (!LittleFS.exists(filename)) {
                        ...
                        if (!deleteLittleFSRecursive(filename)) {
        ...
        if (path != "/" && !LittleFS.exists(path)) {
            String s = "{\"status\":\" ";
            s += path;
            s += " does not exist on Local file system\"}";
    ...
    //LittleFS files uploader handle
    void Web_Server::LittleFSFileupload() {
            ...
            if ((_upload_status != UploadStatusType::FAILED) || (upload.status == UPLOAD_FILE_START)) {
                ...
                if (upload.status == UPLOAD_FILE_START) {
                    ...
                    if (fsUploadFile) {
                        ...
                        //check size
                        String sizeargname = upload.filename + "S";
                        fsUploadFile       = LittleFS.open(filename, FILE_READ);
    ...
    //Function to delete not empty directory on SD card
    bool Web_Server::deleteLittleFSRecursive(String path) {
        bool result = true;
        ...
        while (true) {
            ...
            String entryPath = entry.name();
            String fullName  = path + "/" + entryPath;
            if (entry.isDirectory()) {
                entry.close();
                if (!deleteLittleFSRecursive(fullName)) {
                    result = false;
                }
            } else {
                entry.close();
        ...
        file.close();
        return result ? LittleFS.rmdir(path) : false;
    }
